...
international_clust <- international_visits %>% 
  select(-c(quarter, age, sample)) %>%
  filter(year %in% c(2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019),
         !country %in% c("USA", "Germany")) %>% 
  group_by(country) %>% 
  ...
  # filter(total_visits > 500,
  #        spend_per_visit > 0.5) %>% 
  select(total_visits, spend_per_visit)
...
k_clusters <- tibble(k = 1:63) %>% 
  mutate(kclust = map(k, ~ kmeans(international_clust,
                                  centers = .x,
                                  nstart = 25)),
         tidied = map(kclust, tidy),
         glanced = map(kclust, glance),
         augmented = map(kclust, augment, international_summary))
...
```{r}
fviz_nbclust(international_clust,
             kmeans,
             method = "wss", #within sum of squares
             nstart = 25)
```
...
clusterings %>% 
  unnest(augmented) %>% # Unnest augmented created in step 5.1
  filter(k == 2) %>% # Set number of clusters
  ggplot(aes(x = total_visits, y = spend_per_visit, colour = .cluster)) +
  geom_point()
```
