...
```{r}
domestic_2017_2019 <- regional_domestic_tourism %>% 
  filter(date_code == "2017-2019",
         region_of_residence == "All of GB") %>% 
  select(expenditure)
  

international_2017_2019 <- international_visits %>%
  select(-c(quarter, age, sample)) %>%
  filter(year %in% c(2017, 2018, 2019)) %>% 
  summarise(total_visits = sum(visits),
...
tibble(a = c("Domestic", "International"),
       b = c(domestic_2017_2019$expenditure, international_2017_2019$total_spend)) %>% 
  ggplot() +
  geom_col(aes(x = a, y = b)) +
  labs(
    x = "",
